...
export const api = {
  ...
  ,hrSalaryHistory: (employeeId: string) => request<SalesRow[]>(`/hr/employees/${employeeId}/salary-history`)
  ,createPayrollRun: (runMonth: string) => request<SalesRow>('/hr/payroll-runs', { method: 'POST', body: JSON.stringify({ runMonth }) })
  ,updatePayrollItem: (itemId: string, data: Record<string, unknown>) => request<SalesRow>(`/hr/payroll-items/${itemId}`, { method: 'PATCH', body: JSON.stringify(data) })
  ,approvePayroll: (runId: string, stage: string) => request<SalesRow>(`/hr/payroll-runs/${runId}/approve`, { method: 'POST', body: JSON.stringify({ stage }) })
  ...
  ,paySalary: (itemId: string, paymentMode: string, paymentReference = '', paymentReceipt = '') => request<SalesRow>(`/hr/payroll-items/${itemId}/pay`, { method: 'POST', body: JSON.stringify({ paymentMode, paymentReference, paymentReceipt }) })
  ...
  ,approvePurchaseOrder: (id: string, approve: boolean) => request<SalesRow>(`/procurement/purchase-orders/${id}/approve`, { method: 'POST', body: JSON.stringify({ approve, comment: approve ? 'Approved through procurement workspace.' : 'Rejected through procurement workspace.' }) })
  ,payVendorBill: (id: string, amount: number, paymentMode: string, transactionReference: string) => request<SalesRow>(`/procurement/bills/${id}/pay`, { method: 'POST', body: JSON.stringify({ amount, paymentMode, transactionReference }) })
  ,approvePettyCash: (id: string, approve: boolean) => request<SalesRow>(`/procurement/petty-cash/${id}/approve`, { method: 'POST', body: JSON.stringify({ approve, comment: approve ? 'Approved through finance workspace.' : 'Rejected through finance workspace.' }) })
  ,payPettyCash: (id: string) => request<SalesRow>(`/procurement/petty-cash/${id}/pay`, { method: 'POST' })
  ,reversePettyCash: (id: string, reason: string) => request<SalesRow>(`/procurement/petty-cash/${id}/reverse`, { method: 'POST', body: JSON.stringify({ reason }) })
